...
### HTTP 請求
<APIEndpoint method="GET" url="/v5/position/move-history" />

### 請求參數
| 參數 | 是否必需 | 類型 | 說明|
|:----- |:-------|:-----|------ |
|[category](../enum#category) |false |string |產品類型 `linear`, `spot`, `option`, `inverse`|
...
|> [category](../enum#category) |string |產品類型<ul><li>[統一帳戶2.0](../acct-mode#統一帳戶20), [統一帳戶1.0](../acct-mode#統一帳戶10): `linear`, `spot`, `option`</li></ul>|
|> orderId |string |Waanx側的訂單ID|
|> userId |integer |用戶ID|
|> symbol |string |合約名稱|
...
|> rejectParty |string |<ul><li>`""`表示初始校驗通過, 需要進一步通過[查詢移倉歷史](move-position-history#)接口來確認最終狀態</li><li>`Taker`, `Maker`: 當status=`Rejected`返回</li><li>`waanx`表示處理過程中的錯誤發生在Waanx側</li></ul> |
