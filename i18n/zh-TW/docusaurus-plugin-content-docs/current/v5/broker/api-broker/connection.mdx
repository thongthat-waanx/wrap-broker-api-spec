...
- 相較共享 stream.waanx.com 路由，延遲更低且更穩定。
- 提升下單、改單與撤單的執行一致性。
- 改善私有 WebSocket 推送交付。
- 在高流量期間改善低延遲行情資料接入。
...
使用 Waanx 提供的專屬 hostname，並保持相同的 V5 WebSocket 路徑結構:
...
如需諮詢經紀商資格，請聯繫 `broker_program@waanx.com`。

## 申請流程

經 Broker Account Manager 批准後，請聯繫 `api_services@waanx.com` 或您的專屬經紀商支援群組，並提供以下資料:
...
3. 透過您的 Broker Account Manager、`api_services@waanx.com` 或專屬經紀商支援群組提交申請資料。
4. 審核通過後，針對支援路徑，將公共 hostname 替換為 Waanx 提供的專屬 hostname。
