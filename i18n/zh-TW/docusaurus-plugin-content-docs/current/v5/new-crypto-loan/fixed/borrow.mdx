...
### HTTP 請求
<APIEndpoint method="POST" url="/v5/crypto-loan-fixed/borrow" />
...
|autoRepay |false|string |已廢棄。啟用「自動還款」可在借款訂單到期時，自動使用資金帳戶中的資產還款，以避免逾期罰款。請確保資金帳戶中有足夠金額，以避免自動還款失敗。<br/>`"true"`：啟用，預設值；`"false"`：停用|
