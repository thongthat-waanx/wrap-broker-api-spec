...
:::tip
若入參未傳 `txnId` 也未傳 `orderNo`，則 `type` 為必填項。傳入 `txnId` 或 `orderNo` 時不受此限制。
:::

### HTTP 請求
<APIEndpoint method="POST" url="/v5/card/transaction/query-asset-records" />
...
url = "https://waanx.com/v5/card/transaction/query-asset-records"
headers = {
    "X-BAPI-API-KEY": "xxxxxxxxxxxxxxxxxx",
    "X-BAPI-SIGN": "XXXXX",
    "X-BAPI-TIMESTAMP": "1672211918471",
...
const url = 'https://waanx.com/v5/card/transaction/query-asset-records';
const headers = {
  'X-BAPI-API-KEY': 'xxxxxxxxxxxxxxxxxx',
  'X-BAPI-SIGN': 'XXXXX',
  'X-BAPI-TIMESTAMP': '1672211918471',
