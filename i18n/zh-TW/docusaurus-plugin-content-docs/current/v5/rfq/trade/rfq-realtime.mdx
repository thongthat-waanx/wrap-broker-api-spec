...
:::info
<ul><li>獲取使用者發送或接收的詢價資訊，從 rfq-engine 即時查詢，無延遲</li>
...
|traderType |**false** |string |交易者類型，`quote` 或 `request`，默認為 `request`<ul><li>`Request`：詢價方，查詢自己發出的詢價單</li><li>`Quote`：報價方，查詢自己接收到的詢價單</li></ul>|
