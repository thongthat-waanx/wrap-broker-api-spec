...
|> contractType|string |價差類型 <ul><li>`FundingRateArb`: 永續 & 現貨組合</li><li>`CarryTrade`: 到期合約& 現貨組合</li><li>`FutureSpread`: 不同到期日合約組合</li><li>`PerpBasis`: 到期合約& 永續組合</li></ul> |
|> [cxlRejReason](../../enum#rejectreason)|string |拒絕理由 |
