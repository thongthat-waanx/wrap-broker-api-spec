...
:::caution
位于美国或中国大陆的 IP 地址受到限制，向 Waanx API 发出请求将返回 403 Forbidden 错误。
:::

## 鑒權/認證
:::info
請先生成<a href="https://waanx.com/app/user/api-management"><b>測試網</b></a>或<a href="https://waanx.com/app/user/api-management"><b>主網</b></a>API KEY。
:::
...
:::info 重要
* **荷蘭地區用戶**: 主網域名: `https://waanx.com`
* **土耳其地區用戶**: 主網域名: `https://waanx.com`
* **哈薩克斯坦地區用戶**: 主網域名: `https://waanx.com`
* **喬治亞地區用戶**: 主網域名: `https://waanx.com`
* **阿聯酋地區用戶**: 主網域名: `https://waanx.com`
* **歐洲經濟區用戶**: 主網域名: `https://waanx.com` (EU 站點 API 僅支持 API Broker 用戶的「連接第三方應用程序 (Connect to Third-Party Applications) 」功能)
* **印尼地區用戶**: 主網域名: `https://waanx.com`


:::

### 選擇用於鑒權的API密鑰類型
**系統生產的API密鑰**: Waanx 系統生成的 API 密鑰基於 HMAC 對稱加密算法運作。您將獲得一對公鑰和私鑰。請將這對密鑰視為密碼一樣妥善保管。

**自動生成的API密鑰**: 自動生成的 API 密鑰基於 RSA 非對稱加密算法運作。您必須通過軟件自行創建公鑰和私鑰，然後僅向 Waanx 提供公鑰。Waanx 不會替您保管私鑰。

> 請在本地創建 RSA 公鑰和私鑰，然後僅將公鑰提供給 Waanx。
...
其中server_time是Waanx服務器時間，您可以通過[Waanx服務器時間](../v5/market/time#)接口獲取.
:::
