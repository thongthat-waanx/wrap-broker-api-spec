...
* 如果因為Waanx服務原因，會重新發送snapshot報文，該報文已保證是最新且準確的.
:::
...
|> u |integer |更新id <ul><li>一般情況下該id是連續的。偶爾會因後台的重啟而發送"u"=1的全量數據，接收到後請覆蓋本地保存的orderbook</li><li>對於期貨的1檔推送, 3秒內無變化, 則會強推一個snapshot數據, 此消息裡的"u"的值和前一條消息裡的保持一致</li></ul>|
