...
|> parentOrderLinkId |string |表示關聯到的母訂單, 用於關聯附帶的止盈(Take Profit)與止損(Stop Loss)訂單. 支援期貨與期權. <ul><li>對止盈或止損訂單進行[修改](../../order/amend-order#)不會改變parentOrderLinkId</li><li>**期貨**: 使用[設置止盈止損](../../position/trading-stop#)修改從原始訂單更新附帶的TP/SL, 不會改變 parentOrderLinkId<br/>**期權**: 使用[設置止盈止損](../../position/trading-stop#)修改從原始訂單更新附帶的TP/SL, 會改變 parentOrderLinkId<br/>**期貨與期權**: 若對原本沒有附帶 TP/SL 的倉位，透過[設置止盈止損](../../position/trading-stop#)設定 TP/SL, 則parentOrderLinkId沒有實際意義</li></ul> |
|> isLeverage |string |是否借貸. 僅`spot`有效<ul><li>`0`: 否, 幣幣交易</li><li> `1`: 是, 槓桿交易</li></ul>|
...
|> [createType](../../enum#createtype) |string |訂單創建類型<ul><li>僅作用於category=linear 或 inverse</li><li>現貨、期權不返回該字段</li></ul> |
...
|> cumExecFee |string |<ul><li>`inverse`, `option`: 訂單累計成交的手續費.</li><li>`linear`, `spot`: 已棄用. 用`cumFeeDetail`替代.</li><li>升級到統一帳戶後, 您可以使用[成交](execution)頻道中的`execFee`字段來獲取每次成交的手續費</li></ul> |
...
|> cumFeeDetail |json |<ul><li>`linear`, `spot`: 累積交易費詳情, 替代`cumExecFee`</li></ul>` |
