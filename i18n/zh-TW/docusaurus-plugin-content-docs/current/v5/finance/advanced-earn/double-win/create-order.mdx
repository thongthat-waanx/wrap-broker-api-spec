...
* **申購（Stake）**：必須傳入 `doubleWinStakeExtra`。固定區間產品請使用[查詢固定產品報價](./product-quote)中的 `leverage`；RFQ 產品請使用[查詢自選區間產品報價](./leverage)中的 `leverage`，並傳入 `lowerPrice` 和 `upperPrice`。須在報價的 `expireTime` 前完成下單。
...
|> leverage|**true**|string|槓桿倍數，從[查詢固定產品報價](./product-quote)或[查詢自選區間產品報價](./leverage)獲取。最多 2 位小數，不可超過報價返回的最大值|
|> initialPrice|**true**|string|下單時的標的資產指數價格。建議使用[查詢固定產品報價](./product-quote)返回的 `currentPrice`|
|> lowerPrice|false|string|**僅 RFQ 產品需填寫。** 自選價格區間下限，須與[查詢自選區間產品報價](./leverage)中使用的值一致|
|> upperPrice|false|string|**僅 RFQ 產品需填寫。** 自選價格區間上限，須與[查詢自選區間產品報價](./leverage)中使用的值一致|
