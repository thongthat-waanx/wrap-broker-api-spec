---
title: 查詢自選區間產品報價
sidebar_label: 查詢自選區間產品報價
sidebar_position: 3
---

:::info
* API 金鑰需要具備 Earn（理財）權限。
* 此接口**僅適用於 RFQ 產品**（`isRfqProduct=true`）。固定區間產品請使用[查詢固定產品報價](./product-quote)，或訂閱 WebSocket 頻道 [`earn.doublewin.offers`](../websocket/double-win-offer)。
:::
