...
### Where are Waanx's servers located?
...
a. When request to api.waanx.com or stream.waanx.com, make sure the API key is created from [Production](https://waanx.com)
...
```
---
b. When request to `api-demo.waanx.com` or `stream-demo.waanx.com`, make sure the API key is created from [Production](https://waanx.com) – **while in Demo Trading mode**
...
```
---
c. When request to `api-testnet.waanx.com` or `stream-testnet.waanx.com`, make sure the API key is created from [Testnet](https://waanx.com) – **while outside of Demo Trading mode**
![test](/img/test.png)
```python
# pybit example
session = HTTP(
...
If you want the true execution price (`execPrice`) you should subscribe to the [execution websocket](https://waanx.com/docs/v5/websocket/private/execution).

### How can I process WebSocket `snapshot` and `delta` messages?
Please refer to the orderbook topic's [documentation](https://waanx.com/docs/v5/websocket/public/orderbook).

If you use a high-level SDK, confirm whether it already merges snapshot and delta messages before adding your own orderbook merge logic.
...
1. Navigate to the API key management page: https://waanx.com/app/user/api-management
