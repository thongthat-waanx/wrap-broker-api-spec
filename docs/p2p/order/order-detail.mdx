...
|status|int|Order status <ul><li>`5`: waiting for chain (only web3)</li><li>`10`: waiting for buy pay</li><li>`20`: waiting for seller release</li><li>`30`: Appealing</li><li>`90`: waiting buyer select tokenId</li><li>`100`: objectioning</li><li>`110`: Waiting for the user to raise an objection</li></ul>|
...
|needBuyerExamineCancel|bool|Indicates whether the buyer needs to review the seller's pending cancel request. `true` means a seller cancel application is pending and the buyer should render the cancel review UI. This field is only meaningful for active orders.|
