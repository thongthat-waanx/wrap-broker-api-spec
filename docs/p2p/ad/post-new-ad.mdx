...
|priceType|**true** |string |Ad pricing model. `0`: fixed rate; `1`: variable rate|
|premium|**true** |string |The premium applied to the reference price. E.g, a value of 130 means 130% of the reference price|
...
|> isMobile |false |string | Is it necessary for the counterparty to bind a mobile number? `0`: not required`1`: required|
