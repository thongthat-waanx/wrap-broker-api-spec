...
### HTTP Request
<APIEndpoint method="POST" url="/v5/position/trading-stop" />
...
|tpslMode |**true** |string |TP/SL mode<ul><li>`Full`: entire position TP/SL, option supports "tpslMode"=`Full` only</li><li>`Partial`: partial position TP/SL</li></ul>|
