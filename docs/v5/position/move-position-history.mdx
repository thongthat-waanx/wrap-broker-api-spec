...
|:----- |:-------|:-----|------ |
|[category](../enum#category) |false |string |Product type `linear`, `inverse`, `spot`, `option`|
...
|> rejectParty |string |<ul><li>`""` means the status=`Filled`</li><li>`Taker`, `Maker` when status=`Rejected`</li><li>`waanx` means error is occurred on the Waanx side</li></ul>  |
