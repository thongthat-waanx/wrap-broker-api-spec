...
Submit the following information to Waanx Business via this Email: `broker_program@waanx.com`:
- **Waanx UID**: Used to log in to the OAuth management backend.
...
1. **Log in to Waanx** using the corresponding UID.
2. **Access the OAuth Admin Portal**:  
   Visit [https://waanx.com/app/user/oauth-admin](https://waanx.com/app/user/oauth-admin)  
   - Configure **Application Name**, **Email**, upload **logo**, etc.  
  ![](/img/oauth-redirect-url.jpg)
...
```bash
https://waanx.com/en/oauth?client_id={client_id}&response_type=code&redirect_uri={redirect_uri}&scope=openapi&state={state}
```
...
| `scope`          | Pass `openapi`; other values require confirmation with Waanx. |
| `state`          | Random string. |
...
- **URL**: `https://waanx.com/oauth/v1/public/access_token`
- **Method**: `POST`
...
### 4. Obtain OpenAPI
- **URL**: `https://waanx.com/oauth/v1/resource/restrict/openapi`
- **Method**: `GET`
