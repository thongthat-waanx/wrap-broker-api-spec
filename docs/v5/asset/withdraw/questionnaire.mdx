...
The `questionnaire` field is a JSON string used to carry Travel Rule compliance information in both the [Submit Deposit Originator Info](../deposit/submit-info#) and [Withdraw](/v5/asset/withdraw) endpoints. It replaces the legacy `beneficiary` + `transactionPurpose` combination for new integrations.
...
#### WAANX-IND — India (FIU-IND)

Common fields only. No additional required fields.

#### WAANX-KOR — Korea (SAFA) — Withdrawal only
...
|WAANX-IND |India |✅ |✅ |FIU-IND |
|WAANX-KOR |Korea |✅ |❌ |SAFA — company representative required for withdrawal; deposit handled via local bank / CTP |
...
#### WAANX-IND — Individual + Non-Custodial Wallet
```json
{
  "walletType": 1,
  "legalType": "individual",
...
#### WAANX-KOR — Company + Custodial Wallet (Withdrawal only)
```json
{
  "walletType": 0,
  "vaspCode": "EXAMPLE_VASP",
