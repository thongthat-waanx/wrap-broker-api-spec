...
Based on the websocket private connection and heartbeat mechanism, Waanx provides disconnection protection function. The
timing starts from the first disconnection. If the Waanx server does not receive the reconnection from the client for
...
### HTTP Request
<APIEndpoint method="POST" url="/v5/order/disconnected-cancel-all" />
