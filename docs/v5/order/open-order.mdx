...
|openOnly |false |integer |<ul><li>`0`(default): query open status orders (e.g., New, PartiallyFilled) **only**</li><li>`1`: Query a maximum of recent 500 closed status records are kept under each account each category (e.g., Cancelled, Rejected, Filled orders).<br/><i>If the Waanx service is restarted due to an update, this part of the data will be cleared and accumulated again, but the order records will still be queried in [order history](order-list#)</i></li><li>`openOnly` param will be ignored when query by _orderId_ or _orderLinkId_</li></ul>|
