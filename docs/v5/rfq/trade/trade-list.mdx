...
### HTTP Request
<APIEndpoint method="GET" url="/v5/rfq/trade-list" />
...
|>>> rejectParty |string |Empty if status is `Filled`. Valid values: `Taker` or `Maker` if status is `Rejected`, "rejectParty='waanx'" to indicate errors that occur on the Waanx side.|
