...
Query for [ADL](https://waanx.com/en/help-center/article/Auto-Deleveraging-ADL) (auto-deleveraging mechanism) alerts and insurance pool information.
...
1. **Example 1: Pool has no significant profit in the last 8 hours, then symbol loss exceeds the PnL ratio threshold (`insurancePnlRatio`), ADL will be triggered**
   ...
   - Calculation:
     - `pnlRatio` = -35%
     - `balance` = 1M
     - `adlTriggerThreshold` = 1 (a constant set by Waanx)
     - `insurancePnlRatio` = -0.3 (a constant set by Waanx)
   - Condition check:
   ...
   - **Stop condition**: ADL stops if A's `pnlRatio` > `adlStopRatio` (-0.25, a constant set by Waanx)
...
Subscribe to the [ADL WebSocket topic](/v5/websocket/public/adl-alert) for faster updates.

### HTTP Request
<APIEndpoint method="GET" url="/v5/market/adlAlert" />
