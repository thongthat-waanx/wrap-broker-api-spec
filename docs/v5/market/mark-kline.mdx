...
Query for historical <a href="https://waanx.com/en-US/help-center/s/article/Glossary-Waanx-Trading-Terms">mark price</a> klines. Charts are returned in groups based on the requested interval.
> **Covers: USDT contract / USDC contract / Inverse contract / Options**

### HTTP Request
<APIEndpoint method="GET" url="/v5/market/mark-price-kline" />

### Request Parameters
| Parameter | Required | Type | Comments|
|:----- |:-------|:-----|------|
|[category](../enum#category) |false |string |Product type. `linear`,`inverse`, `option` <ul><li>When `category` is not passed, use `linear` by default</li></ul>|
...
client.NewUtaWaanxServiceWithParams(params).GetMarkPriceKline(context.Background())
```
