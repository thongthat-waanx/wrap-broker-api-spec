...
Query recent public trading history in Waanx.
> **Covers: Spot / USDT contract / USDC contract / Inverse contract / Option**

You can download archived historical trades from the [website](https://waanx.com/derivatives/en/history-data)

### HTTP Request
<APIEndpoint method="GET" url="/v5/market/recent-trade" />
...
|baseCoin |false |string |Base coin, uppercase only <ul><li>Apply to `option` **only**</li><li>If the field is not passed, return **BTC** data by default</li></ul>|
...
client.NewUtaWaanxServiceWithParams(params).GetPublicRecentTrades(context.Background())
```
