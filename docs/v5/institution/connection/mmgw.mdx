...
Use the dedicated hostname provided by Waanx with the same HTTP path structure:
...
To request MMGW access, provide the IP addresses or CIDR ranges that will send API requests through the gateway. Waanx supports up to 10 IP entries or CIDR ranges for whitelisting. CIDR ranges are recommended when your infrastructure may scale or rotate within a fixed network block.

After approval, Waanx will provide private MMGW hostnames for your account. Keep these hostnames confidential. If malicious traffic is detected through the assigned hostnames, Waanx may disable them urgently.
