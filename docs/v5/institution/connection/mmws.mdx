...
Use the dedicated hostname provided by Waanx with the same WebSocket path structure:
...
To request MMWS access, provide the IP addresses or CIDR ranges that will connect to MMWS. Waanx supports up to 10 IP entries or CIDR ranges for whitelisting. CIDR ranges are recommended when your infrastructure may scale or rotate within a fixed network block.

After approval, Waanx will provide private MMWS hostnames for your account. Keep these hostnames confidential. If malicious traffic is detected through the assigned hostnames, Waanx may disable them urgently.
...
2. Send your source IP addresses or CIDR ranges to your Waanx contact for whitelisting.
3. Replace the public WebSocket hostname with the dedicated MMWS hostname provided by Waanx.
